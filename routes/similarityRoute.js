...
router.post("/calculate-matches", authenticateUser, async (req, res) => {
  try {
    ...
    await MatchResult.findOneAndUpdate({ userId: userId }, matchDocument, {
      upsert: true,
      new: true,
    });
    res.status(200).json({
...
router.get("/matches/", authenticateUser, async (req, res) => {
  try {
    ...
    const { minSimilarity = 0, limit = 10, page = 1 } = req.query;

    const skip = (page - 1) * limit;

    const matchResults = await MatchResult.findOne({ userId }).populate({
      ...
      .filter((match) => {
        ...
        if (maxDistance) {
          if (
            ...
            Array.isArray(matchedUser.location.coordinates)
          ) {
        ...
        console.log("here after distance");
        return true;
      })
      .sort((a, b) => b.similarityScore - a.similarityScore)
      .slice(skip, skip + parseInt(limit));
